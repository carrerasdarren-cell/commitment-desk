...
The live Nova Lite sample check passed on September 10, 2026: all five expected attention states, all nine evidence records, zero rejected events, and real read/structured-output tool execution for all three sources. Twenty-nine tests cover domain logic, extraction schemas, the adapter, and protected judge admission. This is a bounded synthetic sample, not a general accuracy benchmark or a completed hackathon submission. See `docs/validation.md` for runtime and publication verification.

## Local setup

Requires Node 22.18+ on the Node 22 line, or Node 24+. The admission tests use Node's built-in SQLite module. This project was tested with **Node 26.0.0** and npm; `.node-version` records that version. The verified provider is Amazon Nova Lite 1.0 in AWS Bedrock, region `us-east-2`.
...
The app uses a server route and stores no project notes in a database. The server and configured AWS model process submitted notes. The application does not store notes or results in D1; review state stays in browser memory and exports are local downloads. Do not treat exact quote matching as a guarantee of semantic accuracy: the model can still group tasks incorrectly, misinterpret intent, or miss information. Human review remains essential.

Cloud deployment requires a reachable model provider. A hosted server cannot reach the developer's local Ollama service. Production live analysis is disabled until both `MODEL_PROVIDER` and `LIVE_ACCESS_MODE` are explicitly configured. The Bedrock adapter uses `BEDROCK_MODEL_ID`, `AWS_REGION`, server-only AWS credentials (or the SDK credential chain locally), and a fetch transport compatible with Workers. The dedicated `commitment-desk-bedrock` identity has no console access and only the Nova Lite invocation permissions in `docs/bedrock-policy.json`. Hosted credentials are Sites secrets. The [public demo](https://commitment-desk-afh-2026.drc795.chatgpt.site/) is open, with live analysis restricted by a private judge code and a shared allowance of 25 admitted reviews through October 8. For private owner access, `LIVE_ACCESS_MODE=owner` retains a per-instance concurrency guard. The hosted demo uses the protected judge mode described in `docs/judge-access.md`. Judge mode uses a secret code, expiry and a durable D1 allowance shared across server instances. The database stores only an attempt count and lease metadata; project notes are never persisted there. Started failures consume allowance. A review-count limit is not an AWS dollar cap.
