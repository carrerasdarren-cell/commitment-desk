...
Earlier Ollama integration attempts did not pass. The first Nova sample exposed unsupported header metadata and null formatting; the current checks cover those failures. A passing sample does not establish general extraction accuracy, public availability, a hackathon submission, or a prize outcome. Human evidence review remains required. The site was initially owner-only. Public access with protected judge mode was activated on September 11 as recorded below. Owner mode is no longer active on the hosted demo.
...
The recorded export button displayed a confirmation, but its browser download event timed out and no file was independently verified at recording time. A later user-assisted download was checked separately as described below. The 3:04 demo labels this later verification accurately; it uses actual browser frames with edited timing and generic synthetic narration. Its full video/audio decode passed. No native Codex interface was captured, and opening the downloaded file is not shown. See evidence/browser-review.json. The [demo is publicly available on YouTube](https://www.youtube.com/watch?v=zkPgiwAC5kY) after owner approval. YouTube reported no issues in its upload checks, and watch-page playback was verified. See `evidence/video-publication.json`.
...
## Public judge activation — September 11, 2026

Darren approved the public demo page, private code and 25 total admitted reviews through October 8. The saved application version was redeployed with environment revision 3 in judge mode while still owner-only. Missing and wrong codes returned 401; malformed sources with the correct code returned 400. One live Nova activation review passed in 12 seconds with all five attention states, nine quotes, zero rejected events, and three successful read/output tool pairs. D1 readback showed `used=1`, a released lease and 24 remaining at verification.

After the audience changed to public, anonymous HTTP checks returned the application page and judge availability without an owner cookie or token. Missing/wrong codes returned 401 and valid-code malformed input returned 400. The first request immediately after the audience change returned 403; subsequent independent root and API checks succeeded. No additional model calls were made for public reachability checks.

Evidence: `evidence/judge-agent-run.json` and `evidence/judge-activation.json`. The real code is excluded from public evidence and is saved only as a server secret and in the private Devpost testing field. The production application commit remains `a4a23a36995421eb6573030482e1d4b886397935`; these are environment, audience and documentation changes. The competition entry still awaits final submission.
